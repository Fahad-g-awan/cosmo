...
export const WorkingHoursSection = () => {
  ...
  const workingHoursError = useFormError("workingHours");
  ...
  const handleEditWorkingHours = useCallback(() => {
    const snapshot = workingHours;
    setSavedWorkingHours(snapshot);
    openDialog({
      ...
      payload: {
        ...
        confirmLabel: "Save",
        onConfirm: () => {
          if (registry.commit("working-hours-section")) {
            closeDialog();
          }
        },
        onCancel: () => setValue("workingHours", snapshot),
      },
    });
  }, [openDialog, setValue, closeDialog, registry, workingHours]);
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <FieldError error={workingHoursError} />
      </div>
