...
const ManagerProfile = () => {
  ...
  const handleFetchManager = useCallback(async () => {
    ...
    try {
      ...
      Toaster("An error occurred while loading manager data", "error");
    } finally {
      setIsLoading(false);
    }
      ...
      payload: {
        onConfirm: async () => {
          try {
            ...
            console.log(
              "[clinic_controlPanel_managers_handleDelete] Error while manager delete:",
              error,
            );
  ...
  useGatedPanelHeader(
    panelHeaderConfig.clinic.controlPanel.managers.record,
    "clinic_manager",
  );
