...
import { buildBlogCategoriesBrowseLayoutConfig } from "../../config/browseLayout.config";
import { BlogCategoryColumnsData } from "../../config/tables.config";
...
const BlogsCategoriesContent = () => {
  ...
  const config = useMemo(() => buildBlogCategoriesBrowseLayoutConfig(), []);
  const lastConfigKey = useRef<string | null>(null);

  const handleFetchBlogsCategories = useCallback(
    async (params: FetchParams): Promise<FetchResponse<BlogCategory>> => {
      ...
      try {
        const res = await getManagementBlogCategoriesApi(
          {
            pagination: {
  ...
  const handleDelete = useCallback(
    (blogCategoryId: string) => {
      if (!blogCategoryId) {
        console.log(
          "[Blogs_Categories_HandleDelete] Error blog category id not provided",
        );
        Toaster(
          "Something went wrong",
          "error",
  ...
  return (
    <BrowseContent
      columns={BlogCategoryColumnsData(handleDelete, viewMode, perms)}
      enableColumnPinning={true}
      initialPinnedColumns={{
        left: ["category-name"],
        right: ["actions"],
      }}
      fetchData={handleFetchBlogsCategories}
      exportConfig={{ slug: "blog-categories" }}
    />
  );
};
...
const BlogsCategories = () => {
  useGatedPanelHeader(
    panelHeaderConfig.admin.blogManagement.categories.main,
    "blog_category",
  );

  return (
    <>
      <BlogsCategoriesContent />
      <DialogRenderer />
    </>
  );
};
