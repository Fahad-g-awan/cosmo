...
export const ManagerFormSchema = z.object({
  ...
  const hasSingle = Boolean(data.clinicId?.trim());
  const hasMulti = Array.isArray(data.clinicIds) && data.clinicIds.length > 0;
  if (!hasSingle && !hasMulti) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "Please select a clinic",
      path: Array.isArray(data.clinicIds) ? ["clinicIds"] : ["clinicId"],
    });
  }
});