...
import { Treatment, FAQs } from "@cosmediate/type-utils";
...
const FaqsTab = ({ treatment }: { treatment: Treatment }) => {
  const profile = useTranslations("profile");
  const common = useTranslations("common");
  const faqs = useMemo<FAQWithId[]>(() => {
    if (!treatment?.faqs) return [];

    return treatment.faqs.map((faq, index) => ({
      ...faq,
      id: String(index),
    }));
  }, [treatment]);
  ...
  return (
    <TabsContainer showBookAppointmentCard={false}>
