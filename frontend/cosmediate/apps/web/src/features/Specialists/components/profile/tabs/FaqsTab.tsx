...
import { Specialist, FAQs } from "@cosmediate/type-utils";
...
const FaqsTab = ({ specialist }: { specialist: Specialist }) => {
  const profile = useTranslations("profile");
  const common = useTranslations("common");
  const faqs = useMemo<FAQWithId[]>(() => {
    if (!specialist?.faqs) return [];

    return specialist.faqs.map((faq, index) => ({
      ...faq,
      id: String(index),
    }));
  }, [specialist]);
