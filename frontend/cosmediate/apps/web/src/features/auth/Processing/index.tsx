  ...
  const exchangeCode = useCallback(
    async (code: string) => {
      try {
        if (!code) {
          console.error(
            "[web:/auth/processing] Unauthenticated: auth code not available"
          );
          setError({
        ...
        if (!data || typeof data !== "object") {
          console.error(
            "[web:/auth/processing] exchange-code returned no body",
            { status: res.status }
          );
        ...
        const errCode = (errBody.error ?? "unknown") as
          | AuthErrorCode
          | "unknown";
        console.error("[web:/auth/processing] exchange-code failed", errBody);
        setError({
          code: errCode,
          message: errBody.message ?? userMessageForCode(errCode),
        });
      } catch (err) {
        console.error("[web:/auth/processing] Unexpected error", err);
        setError({ code: "unknown", message: userMessageForCode("unknown") });
      }
    },
    [userMessageForCode],
