...
import { ADMIN_ROUTE_DEFS } from "/opt/nodejs/config/routes/admin.routes.mjs";
...
const PUBLIC_ROUTE_KEYS = new Set([ADMIN_ROUTE_DEFS.BOOTSTRAP.key]);
...
export const handler = async (event) => {
  ...
  try {
    ...
    await reportDevAlert({
      module: "cosmediate-admins",
      error: e,
      config: runtimeConfig,
      event,
    });
    console.error("[admins] handler", e);
