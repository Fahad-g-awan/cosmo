...
export const listActivityLogs = async (ctx) => {
  const { value: query } = validateRequestBody(
    CRUD_ACTIONS.PLATFORM.ACTIVITY_LOGS_LIST,
    ctx.reqBody,
  );
  ...
  return {
    ...
    data: {
      success: true,
      items: items.map(toActivityLogListItem),
      total: null,
      nextToken,
...
export const getActivityLogById = async (ctx) => {
