...
import { BLOG_ROUTE_DEFS } from "/opt/nodejs/config/routes/blog.routes.mjs";
...
export const handler = async (event) => {
  ...
  try {
    ...
    const queryParams = {
      ...
      from: event.queryStringParameters?.from ?? null,
      allowZeroSearchClicks:
        event.queryStringParameters?.allowZeroSearchClicks ?? null,
      allowZeroRating: event.queryStringParameters?.allowZeroRating ?? null,
    };
    ...
    await reportDevAlert({
      module: "cosmediate-blogs",
      error: e,
      config: runtimeConfig,
      event,
    });
    console.error("[blogs] handler", e);
